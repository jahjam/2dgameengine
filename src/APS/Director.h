...
public:
  ...
  void giveProp(const Name actorName, Arena<T> &propArena,
                Arena<Store<T>> &storeArena, TArgs &&...args);
  template <typename T> void removeProp(Name actorName, Arena<T> &propArena);
  template <typename T> bool hasProp(Name &actorName) const;
  template <typename T> T &getProp(Name &actorName) const;

  template <typename T, typename... TArgs> void addScript(TArgs &&...args);
  template <typename T> void removeScript(Arena<T> &scriptArena);
