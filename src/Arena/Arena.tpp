...
template <typename T> Arena<T>::~Arena() {
...
template <typename T> void Arena<T>::_setMaxAlign() {
  _alignment = alignof(max_align_t);
}

template <typename T> size_t Arena<T>::_pos() { return _top; };

template <typename T> size_t Arena<T>::_getMemoryRemaining() {
  return _memoryChunkRemaining;
}

template <typename T> void *Arena<T>::_pushNoZero() {
...
template <typename T> void *Arena<T>::_pushAligner() {
...
template <typename T> void *Arena<T>::_push() {
  void *ptr = _pushNoZero();
  void *alloc = std::memset(ptr, 0, sizeof(T));
  return alloc;
};

template <typename T> void Arena<T>::_pop() {
  size_t size = sizeof(T);
...
template <typename T> void Arena<T>::_clear() {
  if (_curChunk > 0) {
