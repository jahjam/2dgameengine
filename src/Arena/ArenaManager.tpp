...
template <typename T> void *ArenaManager<T>::_allocate() {
...
template <typename T> void *ArenaManager<T>::_deallocate(void *ptr) {
  FreeListNode *firstFreeNode = _freeListNode;
  FreeListNode *newNode =
      static_cast<FreeListNode *>(std::malloc(sizeof(FreeListNode)));
...
template <typename T> void ArenaManager<T>::_clearFreeList() const {
...
template <typename T> void ArenaManager<T>::_clearFreeListAndArena() const {
  _clearFreeList();
  _arena._clear();
}
